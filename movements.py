...
def _ensure_item_clearance(item_id: int, user: Dict[str, Any]) -> None:
    ...
    rows = db.db_read("SELECT clearance_level FROM items WHERE id = ?", (item_id,))
    if not rows:
...
def list_movements(
    ...
    """
    total = db.db_read(total_sql, params)[0]["total"]

    sql = f"""
      SELECT
        m.id,
        m.item_id,
        m.type      AS kind,       -- alias to API/UI
        m.qty       AS quantity,   -- alias to API/UI
        m.shelf_id,
        m.holder,
        m.due_at,
        m.actor_user_id,
...
def receive(payload: ReceiveIn, user=Depends(get_current_user)):
    ...
    mid = db.db_write(
        """
        INSERT INTO movements(item_id, type, qty, shelf_id, actor_user_id, note)
        VALUES (?, 'receive', ?, ?, ?, ?)
        """,
        (payload.item_id, payload.qty, payload.shelf_id, user["id"], payload.note),
    )
...
def do_return(payload: ReturnIn, user=Depends(get_current_user)):
    ...
    mid = db.db_write(
        """
        INSERT INTO movements(item_id, type, qty, shelf_id, holder, actor_user_id, note)
        VALUES (?, 'return', ?, ?, ?, ?, ?)
        """,
        (payload.item_id, payload.qty, payload.shelf_id, payload.holder, user["id"], payload.note),
    )
...
def transfer(payload: TransferIn, user=Depends(get_current_user)):
    ...
    params: Sequence[Sequence[Any]] = [
        (payload.item_id, 'transfer', -payload.qty, payload.from_shelf_id, user["id"], payload.note),
        (payload.item_id, 'transfer',  payload.qty, payload.to_shelf_id,   user["id"], payload.note),
    ]
    db.db_execmany(
        """
        INSERT INTO movements(item_id, type, qty, shelf_id, actor_user_id, note)
